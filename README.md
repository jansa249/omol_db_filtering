...
> [!NOTE]
> If you want to run the analysis scripts in 'Analysis & Validation' on the molecules filtered by the snippet below, you only need to run those. Make sure that `molecule_index.csv` is in the `output_filtered_data/` directory. This file is generated by the `filter_and_extract.py` script, so if you dont want to change the filters, skip the `python filter_and_extract.py` command.
> [!NOTE]
